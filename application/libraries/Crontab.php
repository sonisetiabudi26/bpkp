...
class Crontab
{
	 ...
	 * @return bool
	 */
	function remove_job($time, $action)
	{
		if (is_int($time)) {
			$time = $this->translate_timestamp($time);
		}
	 ...
	 * @return bool
	 */
	function replace_time($from_time, $to_time, $action)
	{
	 ...
	 * @return void
	 */
	function remove_all_jobs($action)
	{
		$jobs = explode("\n", read_file($this->cronfile));
	 ...
	 * @return void
	 */
	function _write($text, $mode = FOPEN_WRITE_CREATE_DESTRUCTIVE)
	{
	 ...
	 * @return string
	 */
	function translate_timestamp($timestamp, $format = '') // s i H d m ? Y  but w/o leading zeroz
	{
		if ($format == '') {
